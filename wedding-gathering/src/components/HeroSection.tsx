...
const HeroSection = () => {
  ...
  const [coupleImageSrc, coupleImageLoaded] = useLazyImage('/couple-placeholder.jpg');

  // Simulate image for demo purposes
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      <div className="container mx-auto px-6 z-10 text-center">
        ...
        >
          <div className={`absolute inset-0 ${coupleImageLoaded ? '' : 'lazy-image'}`}>
            {coupleImageSrc && (
              <img 
                src={coupleImageSrc} 
                alt="Para Młoda" 
                className={`w-full h-full object-cover ${coupleImageLoaded ? 'loaded' : ''}`}
              />
            )}
          </div>
        ...
        <h1 className="font-display text-4xl md:text-6xl lg:text-7xl leading-tight mb-6 text-wedding-dark animate-fade-in opacity-0" style={{ animationDelay: '0.3s', animationFillMode: 'forwards' }}>
          Anna & Bartosz
        </h1>
        
        <p className="font-serif text-xl md:text-2xl text-wedding-accent max-w-2xl mx-auto mb-12 animate-fade-in opacity-0" style={{ animationDelay: '0.6s', animationFillMode: 'forwards' }}>
        ...
        <div className="absolute bottom-12 left-0 right-0 flex justify-center animate-fade-in opacity-0" style={{ animationDelay: '1.2s', animationFillMode: 'forwards' }}>
          <button 
